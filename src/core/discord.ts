...
dsclient.on('ready', () => {
    (dsclient.channels.cache.get(process.env.DISCORDCHANNELID) as TextChannel).send('Discord Client ready and logged in as ' + dsclient?.user?.tag + '.');
})
...
dsclient.on('messageDelete', async (message) => {
    ...
    if(message.author.bot && message.author.id != dsclient?.user?.id) return;
    if(message.channel.id !== process.env.DISCORDCHANNELID) return;
...
dsclient.on("messageUpdate", async (oM, nM) => {
    if (!oM.author || !nM.author || oM.author.bot) return;
    if(oM.channel.id != process.env.DISCORDCHANNELID) return;
